...
/**
 * Solve Elastic Net regression using coordinate descent.
 *
 * Minimizes: (1/2N) * ||y - Xβ||² + α * l1Ratio * ||β||₁ + α * (1 - l1Ratio) / 2 * ||β||₂²
 *
 * X should include an intercept column (first column all 1s).
 * Neither the L1 nor L2 penalty is applied to the intercept.
 * Features are standardized internally for numerical stability.
 *
 * The coordinate descent update for feature j (j > 0) is:
 *   β_j = S(ρ_j, α * l1Ratio) / (x_j^T x_j / N + α * (1 - l1Ratio))
 *
 * When l1Ratio = 1, this reduces to Lasso.
 * When l1Ratio = 0, this reduces to Ridge (via coordinate descent).
 */
  ...
  for (let iter = 0; iter < maxIter; iter++) {
    ...
    for (let j = 0; j < p; j++) {
      ...
      if (j === 0) {
        ...
        // Elastic Net update: soft-threshold with modified denominator
        beta[j] = softThreshold(rho, l1Penalty) / (colNormSq[j] + l2Penalty + 1e-15)
      }
...
/**
 * Compute the coefficient path for Elastic Net regression over a range of alpha values
 * at a fixed l1Ratio. Useful for visualizing the trade-off between L1 and L2 penalties.
 */
export function elasticNetPath(
  ...
  for (const alpha of alphas) {
    const result = solveElasticNet(X, y, alpha, l1Ratio)
